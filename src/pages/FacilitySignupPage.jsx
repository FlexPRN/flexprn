...
function FacilitySignupPage() {
  ...
  const allAgreed = FACILITY_AGREEMENTS.every(a => agreementsAccepted[a.key] === true);

  const proceedToAgreements = (e) => {
    e.preventDefault();
    setError('');
    setStep(2);
  };
  ...
  const handleFinalSubmit = async (e) => {
    ...
    try {
      ...
      // Insert signed agreements
      const signedAgreements = FACILITY_AGREEMENTS.map(a => ({
        user_id: authData.user.id,
        user_type: 'facility',
      ...
      <main className="main signup-main">
        <div className="signup-container">
          <div className="signup-header">
            ...
            <p>
              {step === 1 
                ? 'No setup fees. Step 1 of 2.' 
                : 'Step 2 of 2 — Review and accept all agreements'}
            </p>
          </div>
          ...
          {step === 1 && (
            <form className="signup-form" onSubmit={proceedToAgreements}>
              ...
              <button type="submit" className="primary-btn submit-btn">
                Continue to Agreements <ArrowRight size={18} />
              </button>
          ...
          {step === 2 && (
            <form className="signup-form" onSubmit={handleFinalSubmit}>
              <p className="agreement-intro">
                As a healthcare facility using Flexprn, the following agreements must be accepted. 
                These establish the legal framework for our staffing services.
              </p>

              {FACILITY_AGREEMENTS.map(a => {
                const Icon = a.icon;
                return (
                  <label key={a.key} className="agreement-card">
              ...
              <p className="agreement-disclaimer">
                By creating an account, you confirm you have authority to bind your facility 
                to these agreements. Full document text will be available in your dashboard.
              </p>
